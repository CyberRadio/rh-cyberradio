        ...
        if (!workQueue2.empty()) {
            {
                ...
                if (VITAProcess.Encap.enable_vrl_frames) {
                    ...
                    /* optionally swap the endianess of the packet */
                    /* note that getFrameLength reads the size from the headers */
                    /* so it no longer works post byte swap thus the read/save here */
                    size_t frame_bytes = vrl_frame->getFrameLength();
                    uint32_t * temp_ptr = (uint32_t *) vrl_frame->getFramePointer();

                    if (advanced_configuration.byte_swap) {
                        //FIXME: It should not be necessary to force this VRLP but without it
                        //every other packet is reversed. Possibly a bug in the volk library?
                        temp_ptr[0]=0x504c5256;
            	    	volk_32u_byteswap((uint32_t *) vrl_frame->getFramePointer(),frame_bytes/4);
                    }

                    if (unicast_udp_open) {
                    	result = unicast_transmit(uni_server, vrl_frame->getFramePointer(), frame_bytes);
    ...
    try {
    	if (VITAProcess.IFDPacket.enable) {
    		...
    		if (VITAProcess.IFDPacket.enable_stream_identifier) {
    			...
    			standardDPacket->setStreamIdentifier(atoi(_streamMap.streamID.c_str()));
    		}
...
int SinkVITA49_i::createIFContextPacket(BULKIO::PrecisionUTCTime t, int index) {
    ...
    if (runThread) {
        ...
        if (VITAProcess.IFCPacket.enable_stream_identifier)
        	//set the stream ID
        	pkt->setStreamIdentifier(atoi(_streamMap.streamID.c_str()));
