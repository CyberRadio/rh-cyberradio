...
void SourceVITA49_i::RECEIVER() {
    ...
    // optionally reverse the byte order of the array
    if (byte_swap) volk_32u_byteswap((uint32_t *) &vec_char[0],length/4);
    ...
    while (runThread) {
        if (not found_payload_size) {
            if (_offset > 0) {
                ...
                // optionally reverse the byte order of the array
                if (byte_swap) volk_32u_byteswap((uint32_t *) &vrl_vrt_header[0],vrl_vrt_header.size()/4);
                ...
                // optionally reverse the byte order of the array
                if(byte_swap) volk_32u_byteswap((uint32_t *) &vrt_header[0],vrt_header.size()/4);
            ...
            // exit if runThread stopped while blocking
            if (not runThread) {
            ...
            // optionally reverse the byte order of the headers
            if ((payloadSize>0) && (byte_swap))
        	    volk_32u_byteswap((uint32_t *) packet->data(),(BasicVRLFrame::HEADER_LENGTH + BasicVRTPacket::MAX_HEADER_LENGTH)/4);

            // allow for thread interruption
            boost::this_thread::interruption_point();

            //set basicVRTPacket to point to the received packet(i.e. skip the VRL header)
            rebase_pointer_basicVRT(packet);

            // swap IQ of the data(FIXME: very inefficient method)
            if ((payloadSize>0) && (byte_swap)) {
        	    volk_32u_byteswap((uint32_t *) (packet->data() + BasicVRLFrame::HEADER_LENGTH + BasicVRTPacket::MAX_HEADER_LENGTH),(packet->size() - BasicVRLFrame::HEADER_LENGTH - BasicVRTPacket::MAX_HEADER_LENGTH - 4)/4);
    	    	volk_16u_byteswap((uint16_t *) (packet->data() + BasicVRLFrame::HEADER_LENGTH + BasicVRTPacket::MAX_HEADER_LENGTH),(packet->size() - BasicVRLFrame::HEADER_LENGTH - BasicVRTPacket::MAX_HEADER_LENGTH - 4)/2);
...
void SourceVITA49_i::newConnectCallback(const char *connectionId) {
    LOG_INFO(SourceVITA49_i, "New connection received ::"<<connectionId);
    sendSRI=true;
}
...
void SourceVITA49_i::forceSRI() {
	BULKIO::StreamSRI tempSRI;

    /* time between samples (inverse of sample rate) */
    tempSRI.xdelta = (double) 1/1e6;

    /* 0 for Scalar, 1 for Complex */
    tempSRI.mode = (short) 1;

    tempSRI.streamID = "SOURCEVITA49_STREAMID";
    tempSRI.blocking = false;
    tempSRI.hversion = (long) 1;
    BULKIO::PrecisionUTCTime tstamp = bulkio::time::utils::now();
//FIXME - need epoch time
    tempSRI.xstart = (double) tstamp.twsec;

    /* Platinum time code (1 == seconds) */
    tempSRI.xunits = (short) 1;

    /* # frames to be delivered by pushPacket() call; set to 0 for single packet */
    tempSRI.ystart = (double) 0;
    tempSRI.ydelta = (double) 0.001;
    tempSRI.yunits = (short) 1;
    tempSRI.subsize = 0;
    tempSRI.keywords.length(0);

    /* send to force an update */
    mergeRecSRI(tempSRI,true);
    receivedValidSRI = true;
...
bool SourceVITA49_i::canProcessDataPacket() {
    ...
    if (VITA49Processing_override.enable && receivedValidSRI) {
		//cep test - force item size
    	overrideStreamDefinition.data_format.data_item_size = 16;
...
bool SourceVITA49_i::process_data_packet(std::vector<char> *packet) {
    ...
    // check that the payload format is specified
